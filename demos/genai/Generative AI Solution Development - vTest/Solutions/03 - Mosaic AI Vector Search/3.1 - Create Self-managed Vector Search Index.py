...
# MAGIC
# MAGIC In the previous demo, we chunked the raw pdf documents pages into small sections, computed the embeddings and saved it as a Delta Lake table. Our dataset is now ready. 
# MAGIC
...
# MAGIC %pip install -U transformers==4.30.2 "unstructured[pdf,docx]==0.10.30" langchain==0.1.16 llama-index==0.9.3 databricks-vectorsearch==0.22 pydantic==1.10.9 mlflow==2.9.0 flashrank==0.2.0
...
# MAGIC
# MAGIC **🚨IMPORTANT: Endpoint will be created by the instructor. In the code block below, `create_endpoint` function is commented out. Please wait for the instructor to run the code.** 
...
# replace the endpoint name if it is already created for you and ready to be used
...
# MAGIC %md
# MAGIC ## Serach for Similar Content
# MAGIC
...
# MAGIC
# MAGIC After re-ranking you can review the results to check if the order of the results is changed. 
# MAGIC
...
# MAGIC
# MAGIC **🚨 Warning:** Please don't delete the catalog and tables created in this demo as next demos depend on these resources. To clean-up the classroom assets, run the classroom clean-up script in the last demo. 
