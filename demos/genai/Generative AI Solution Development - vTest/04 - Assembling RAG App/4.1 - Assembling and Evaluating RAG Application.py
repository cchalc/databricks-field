...
# MAGIC
# MAGIC Our Vector Search Index is now ready! To build a complete RAG application, it is time to connect all components that you have learned so far and evaluate the peformance of the RAG. 
# MAGIC
...
# MAGIC
# MAGIC * Describe the process of embedding documents using language models and storing them in a vector database.
# MAGIC
# MAGIC * Describe MLflows's Gen AI features.
# MAGIC
# MAGIC
# MAGIC
...
# MAGIC %pip install -U transformers==4.30.2 "unstructured[pdf,docx]==0.10.30" langchain==0.1.16 llama-index==0.9.3 databricks-vectorsearch==0.22 pydantic==1.10.9 mlflow==2.9.0 databricks-sdk==0.12.0 ragas jq
# MAGIC
...
# MAGIC ## Demo Overview
# MAGIC
# MAGIC As seen in the diagram below, in this demo we will focus on the inference section (highlighted in green). The main focus of the previous demos were  Step 1 - Data preparation and vector storage. Now, it is time put all components together to create a RAG application. 
# MAGIC
...
# MAGIC
# MAGIC In this section we will first define the components that we created before. Next, we will set up the retriever component for the application. Then, we will combine all components together. In the final step, we will register the developed application as a model in Model Registery with Unity Catalog.
...
VECTOR_SEARCH_ENDPOINT_NAME = "vs_endpoint_rag"
...
# MAGIC
# MAGIC There are various libraries that can be used for evaluting the RAG. In this demo we will use [RAGAs](https://github.com/explodinggradients/ragas) framework. This library developed specifically for evaluating RAG pipelines. 
# MAGIC
# MAGIC
# MAGIC To evalaute the RAG pipeline, we need an evalution dataset. Typically this set would include **questions**, **ground truth**, **context**, and **answer** generated by the RAG. Ideally, the **ground truth** is handcrafted by human experts. In this demo, we used [RAGAs' Synthetic test set generation function](https://docs.ragas.io/en/stable/getstarted/testset_generation.html#get-started-testset-generation) to synthetically generate the text.
# MAGIC
# MAGIC Below is the evalution dataset that we will be using.
# MAGIC
# MAGIC **📌 Note:** MLflow evaluation can be used for evaluating RAG pipelines. This topic and in-depth evalaution aren't in the scope of this course. These topics are extensively covered in **"Generative AI Evaluation and Governance"**. 
# MAGIC
...
# MAGIC
# MAGIC Let's use RAGAs library to calculate the five main evalution metrics. In this demo, we will calculate following metrics; **faitfulness, answer relevancy, answer correctness, context recall, context precision**.
...
eval_results = evaluate(response_dataset, metrics, llm=chat_model, embeddings = embedding_model)
eval_results
...
# MAGIC
# MAGIC Now that our model is ready and evaluared, we can register it within our Unity Catalog schema. 
# MAGIC
...
# MAGIC ## Conclusion
# MAGIC
# MAGIC In this demonstration, we illustrated the process of constructing a comprehensive RAG application utilizing a variety of Databricks products. Initially, we established the RAG components that were previously created in the earlier demonstrations, namely the Vector Search endpoint and Vector Search index. Subsequently, we constructed the retriever component and set up the foundational model for use. Following this, we put together the entire RAG application. As a final step, we registered the newly created RAG application as a model within the Model Registry with Unity Catalog. 
