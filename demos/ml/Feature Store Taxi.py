...
dbutils.widgets.text("database", "cjc_taxi", "Database name")
...
# MAGIC %md
# MAGIC Set the allow list for Synapse-ML [automatic logging](https://microsoft.github.io/SynapseML/docs/mlflow/autologging/#automatic-logging)
...
dbutils.fs.put("/dbfs/tmp/log_model_allow_list.txt", allow_list, True)

# COMMAND ----------

spark.conf.set("spark.mlflow.pysparkml.autolog.logModelAllowlistFile", "/dbfs/tmp/log_model_allowlist.txt")
...
print(spark.conf.get("spark.sql.files.maxPartitionBytes")) # for input sizing (128000000 = 128MB = default)
...
# MAGIC 1. In this notebook, replace the following `spark.read.format("delta").load("/databricks-datasets/nyctaxi-with-zipcodes/subsampled")` with: `spark.read.table("{database}.nyc_yellow_taxi_with_zips")`
...
fs.create_table(
    name=f"{database}.trip_pickup_features",
    primary_keys=["zip", "ts"],
    df=pickup_features,
...
fs.create_table(
    name=f"{database}.trip_dropoff_features",
    primary_keys=["zip", "ts"],
    df=dropoff_features,
    partition_columns="yyyy_mm",
...
# MAGIC       name=f"{database}.trip_pickup_features",
...
SELECT SUM(count_trips_window_30m_dropoff_zip) AS num_rides,
       dropoff_is_weekend
FROM   {database}.trip_dropoff_features
...
pickup_features_table = f"{database}.trip_pickup_features"
dropoff_features_table = f"{database}.trip_dropoff_features"
...
from synapse.ml.core.platform import materializing_display as m_display
import math
...
features_and_label = training_df.columns
train, test = training_df.randomSplit([0.7, 0.3], seed = 123)
print("There are %d training examples and %d test examples." % (train.count(), test.count()))

feature_cols = features_and_label.remove("fare_amount")
featurizer = VectorAssembler(inputCols=feature_cols, outputCol="features")
# train_data = featurizer.transform(train)["fare_amount", "features"]
# test_data = featurizer.transform(test)["fare_amount", "features"]
...
df_trans = featurize.fit(training_df).transform(training_df)
