...
# MAGIC %md
# MAGIC # Recipe: Multivariate Anomaly Detection with Isolation Forest
# MAGIC This recipe shows how you can use SynapseML on Apache Spark for multivariate anomaly detection. Multivariate anomaly detection allows for the detection of anomalies among many variables or timeseries, taking into account all the inter-correlations and dependencies between the different variables. In this scenario, we use SynapseML to train an Isolation Forest model for multivariate anomaly detection, and we then use to the trained model to infer multivariate anomalies within a dataset containing synthetic measurements from three IoT sensors.
# MAGIC 
# MAGIC To learn more about the Isolation Forest model please refer to the original paper by [Liu _et al._](https://cs.nju.edu.cn/zhouzh/zhouzh.files/publication/icdm08b.pdf?q=isolation-forest).
...
# MAGIC %md
# MAGIC ## Training data preparation
...
# MAGIC %md
# MAGIC Next, we create an ML pipeline to train the Isolation Forest model. We also demonstrate how to create an MLFlow experiment and register the trained model.
# MAGIC 
# MAGIC Note that MLFlow model registration is strictly only required if accessing the trained model at a later time. For training the model, and performing inferencing in the same notebook, the model object model is sufficient.
...
# MAGIC %md
# MAGIC ## Perform inferencing

# COMMAND ----------

# MAGIC %md
# MAGIC Load the trained Isolation Forest Model
...
# MAGIC %md
# MAGIC ## ML interpretability
# MAGIC In this section, we use ML interpretability tools to help unpack the contribution of each sensor to the detected anomalies at any point in time.
...
# MAGIC %md
# MAGIC Display the dataframe with `shapValues` column
...
# MAGIC %md
# MAGIC Retrieve local feature importances
...
# MAGIC %md
# MAGIC Next, install libraries required for ML Interpretability analysis
...
# MAGIC %md
# MAGIC Visualize anomaly results and feature contribution scores (derived from local feature importance)
...
# MAGIC %md
# MAGIC When you run the cell above, you will see the following plots:
# MAGIC 
# MAGIC ![](https://mmlspark.blob.core.windows.net/graphics/notebooks/mvad_results_local_importances.jpg)
# MAGIC 
# MAGIC - The first 3 plots above show the sensor time series data in the inference window, in orange, green, purple and blue. The red vertical lines show the detected anomalies (`prediction` = 1).. 
# MAGIC - The fourth plot shows the outlierScore of all the points, with the `minOutlierScore` threshold shown by the dotted red horizontal line
# MAGIC - The last plot shows the contribution scores of each sensor to the `outlierScore` for that point.

# COMMAND ----------

# MAGIC %md
# MAGIC Plot aggregate feature importance
...
# MAGIC %md
# MAGIC When you run the cell above, you will see the following global feature importance plot:
# MAGIC 
# MAGIC ![](https://mmlspark.blob.core.windows.net/graphics/notebooks/global_feature_importance.jpg)

# COMMAND ----------

# MAGIC %md
# MAGIC Visualize the explanation in the ExplanationDashboard from https://github.com/microsoft/responsible-ai-widgets.
